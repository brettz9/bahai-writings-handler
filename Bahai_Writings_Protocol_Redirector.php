...
abstract class Bahai_Writings_Protocol_Redirector extends Protocol_Redirector {
    ...
    public function parse_query_string () {
        ...
        if (!$matches) {
            $this->report_error("Wrong link format provided; must contain a colon-separated series of language, work abbreviation, and query item (e.g., page number).");
        }
        ...
        // Not used yet but could allow &query=<term> and &action=highlight (ka:5:page:hilite:<freetext-string>)
    ...
    protected function get_params ($query) {
        ...
        $parse_regex = ':([^:]{2,}):([^:]*):([^:]*)(?::([^:]+))?(?::([^:]+))?(?::([^:]+))?';
        preg_match('/^' . preg_quote($this->protocol) . $parse_regex . '$/', $this->query, $matches);
        return $matches;
    }
